...
function mountHelix3D(strand1, strand2, mount, initialIndex = null) {
    ...
    controls.rotateSpeed = -Math.abs(controls.rotateSpeed);
    ...
    function makeLabel(letter, colorHex) {
        ...
        ctx.fillStyle = textColor(colorHex); ctx.strokeStyle = "rgba(0,0,0,0.35)"; ctx.lineWidth = 3;
        ctx.font = "bold 64px sans-serif"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.strokeText(letter, s / 2, s / 2); ctx.fillText(letter, s / 2, s / 2);
    ...
    function setActiveImmediate(vi) {
        ...
        if (active >= 0) {
            ...
            const rPrev = rungMeshes[active]; if (rPrev) rPrev.material.opacity = rungBaseOpacity[active];
        }
        active = vi;
        const p1 = baseMeshes1[vi], p2 = baseMeshes2[vi], r = rungMeshes[vi];
        if (p1 && p2) {
            p1.scale.set(1.6, 1.6, 1.6); p2.scale.set(1.6, 1.6, 1.6);
            ...
            ring1.position.copy(p1.position); ring2.position.copy(p2.position);
            ring1.visible = ring2.visible = true;
            if (r) r.material.opacity = Math.min(1, rungBaseOpacity[vi] * 1.6);
        }
    }
    ...
    function toIndex(seqIndex) {
        ...
        if (vi < 0 || vi >= N) return;
        desiredZ = SLIDE_SIGN * focusZOfVisualIndex(vi);
    ...
    function animate() {
        ...
        if (ring1.visible) {
            ...
            ring1.lookAt(camera.position); ring2.lookAt(camera.position);
        }
    ...
    function resume() { }
    function dispose() {
        ro.disconnect(); renderer.dispose(); mount.innerHTML = "";
        baseGeom.dispose(); rungGeom.dispose(); tubeGeom1.dispose(); tubeGeom2.dispose(); ringGeom.dispose();
    }
